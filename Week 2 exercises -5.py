...
while True: #Start game loop
    ...
    while player_state != 'S' and player_value < 21:
        ...
        if player_value + new_card > 21 and new_card == 11: #Ace Value Card
            new_card = 1
        player_value = player_value + new_card
        
        print("You have drawn a %d," %new_card)
    
        #Ask whether they want to draw another card, or stay
        print("You currently have a total of %d! \n" %player_value)
        if player_value < 21: #This to hide the input if the value has exceed 21 as we don't require input
            player_state = input("To press enter to draw a card or enter S to stay \n").upper()
            
            
    else:  #Win conditions 21 or bust
        if player_value == 21:
            print("You Win!")
